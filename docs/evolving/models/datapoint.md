...
Telegramm zur Zeit x: Installation + 16-Bit-Ziel + **bus-wirksame** Bindings (`bus_ga_bindings`, `bus_pa_bindings`). ETS-Semantik: `E(entity, x) = max(last_modified) <= x` (siehe `.cursor/plans/temporal-bus-semantics.md`).
