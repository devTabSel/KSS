...
description: >-
  ...
  You MUST delegate to Modellierer whenever the task invokes the
  `knx-semantik` skill, `/kss-semantik`, `semantik`, or explicitly mentions
  Modellierer
  ...
  REST, Importer, xknxproject oder Business-Logik.
model: inherit
---
...
8. Keine REST-Endpoints, keine Importer, keine xknxproject-Entwicklung, keine 3API-Schema-Edits.
...
Import gehört zum **Importer** (Skill `knx-semantik`), HTTP zum **APIler**.
