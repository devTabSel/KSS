...
def _upsert_installation_from_info(
    ...
    *,
    ...
    if installation is None:
        installation_index_raw = info.get("installation_index")
        installation_index = (
            int(installation_index_raw) if installation_index_raw is not None else None
        )
        installation = Installation(
            id=uuid4(),
            ets_id=str(info["ets_id"]) if info.get("ets_id") else None,
            project_guid=project_guid,
            knx_project_id=str(info["project_id"]) if info.get("project_id") else None,
