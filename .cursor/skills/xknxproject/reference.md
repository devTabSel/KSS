...
- Device: `ets_id` (`DI-n`), `identifier`, `comment`, `completion_status`, `last_modified`, `last_download` (Sentinel `0001-01-01` → `null`), `serial_number` (Base64→Hex), `*Loaded`, `product_ref`, `hardware_program_ref`, `installation_hints`, `segment_ets_id`. Dict-Key bleibt Individualadresse. Geräte ohne IA weiterhin nicht im Dict. Additive `group_object_tree` (alle GOT-Channels inkl. leer/nested, Folders `PB-*`, ChannelInstance-Join `DI-n_CI-n`) und `comm_objects` (alle COs keyed `O-…_R-…`, auch ohne Links). HA-`channels` und Top-Level-`communication_objects` unverändert (nur genutzte/gelinkte).
