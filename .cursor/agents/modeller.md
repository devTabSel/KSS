...
3. Temporale Semantik berücksichtigen
   ...
   - Für historisierte semantische Attribute ist das bevorzugte Muster: Primärschlüssel `(entity_id, last_modified)`, ohne Surrogat-ID, ohne `valid_to`, ohne GiST-Exclude. Die Gültigkeit einer Version ist `[last_modified, nächstes last_modified)`. Aktuell = `max(last_modified)`. Historische Zeilen werden nicht aktualisiert. Die stabile 3API-UUID bleibt eine eigene Identitätstabelle, damit Fremdschlüssel nicht an eine Version binden.
   - **`last_modified`** (NOT NULL, PK-Teil) ist ETS-Versionsschlüssel und 3API `lastModified` (eine Spalte). Zuweisung kanonisch in `kss.models.temporal` und `plans/temporal-bus-semantics.md`: Objekt-LastModified, sonst Projekt-LastModified. Kein `INITIAL_SINCE`, keine `_since`-Zeilen.
   - **Import-Uhr:** `installations.last_import` (UTC, bei PATCH-Ingest). Ersetzt `_observable_since`. Bei identischem `(entity_id, last_modified)` keine zweite Versionszeile.
   - **BUS:** materialisierte Tabellen `bus_pa_bindings`, `bus_ga_bindings` mit `last_downloaded`; getrennt von ETS-Versionen. Device-Flags `*Loaded` steuern Befüllung.
