...
- Identitätstabelle (3API-UUID) + Version PK `(entity_id, last_modified)`. Import-Uhr: `installations.last_import`. Regeln: `src/kss/models/temporal.py`, `.cursor/plans/temporal-bus-semantics.md`. BUS: `bus_pa_bindings`, `bus_ga_bindings`.
...
Details je Paket in den Dateien dieses Ordners. Import-Regeln: Skill `knx-semantik` (Agent **Importer** füllt auf Anforderung; knxproj-HTTP: **APIler**).
