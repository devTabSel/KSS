...
Nicht aus TTL: Topology `A-*`/`L-*`/`S-*`, Channels/Folders/COs, BUS, GroupRange, knx_master, `core:Functionality`, `core:Datapoint` (COs) als Datapoints, `prj:Site`. Fehlende Entities werden nicht unlinked. Kein Auto-Join `mac:assignedTrade` → `trades.T-n`. JSON-LD-Ingest fehlt.
