...
def test_creates_installation_from_wa53h10_info(session: Session) -> None:
    ...
    assert result.installation.knx_project_id == "P-040E"
    assert result.installation.installation_index == 0
    assert result.installation.group_address_style == "ThreeLevel"
...
def test_identity_fields_are_not_rewritten(session: Session) -> None:
    ...
    assert first.installation.group_address_style == "ThreeLevel"
    assert result.installation.id == first.installation.id
